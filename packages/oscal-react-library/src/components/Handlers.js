...
export function useHandlers() {
  ...
  function fetchValidationWrapper(file) {
    fetchValidation(file, setCurrentValidation, () => {
      return setCurrentValidation([]);
    });
  }
  ...
  const handleAdd = (newPath) => {
    fetchAddSubtree(
      currentFile,
      newPath,
      (data) => fetchAddSubtreeSuccess(data, newPath),
      (e) => fetchAddSubtreeFail(e, newPath),
  ...
  function handleDeleteSuccess(_response) {
